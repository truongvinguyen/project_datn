...
//trừ số lượng
            foreach(Session()->get('cart')->products as $item){
                $sold_old = DB::table('product_inventory')->select('sold','inventory')->where('id',$item['productInfo']->id)->first();

                inventory::find($item['productInfo']->id)->update([
                    'sold'=> $sold_old->sold + $item['quanty'],
                    'inventory' => $sold_old->inventory - $item['quanty']
                ]);
            }
